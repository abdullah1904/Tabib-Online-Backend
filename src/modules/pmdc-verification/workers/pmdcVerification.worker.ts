  ...
  if (name === "process-verification") {
    ...
    try {
      ...
      const response = await pmdcVerificationAgent.invoke({
        ...
        medicalDegree: getMedicalDegreeText(doctor.professionalInfo.medicalDegree)!,
        postGraduateDegree: getPostGraduateDegreeText(doctor.professionalInfo.postGraduateDegree)!,
        messages: [
          {
            role: "user",
...
pmdcVerificationWorker.on("completed", (job: Job) => {
